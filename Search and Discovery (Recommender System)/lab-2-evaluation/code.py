...
if __name__ == '__main__':
	...
	for r in [10,100]:
		#Bias
		eval = MultiEval('expval-bias', recommend=r)
		eval.add_datasets(travalpair, name='ML-Small')
		eval.add_algorithms([Bias(damping=d) for d in [10,100,200]],attrs=['damping'], name=names[0])
		eval.run()
		res = compare('expval-bias',travalpair_binary,['damping'])
		print("--- Explicit Validation Performance ---")
		print(res)
		op1 = res['ndcg'].loc[names[0]].idxmax()
			#BiasedMF
		eval = MultiEval('expval-biasedmf', recommend=r)
		eval.add_datasets(travalpair, name='ML-Small')
		eval.add_algorithms([BiasedMF(features=f) for f in [20,100,200]], attrs=['features'], name=names[1])
		eval.run()
		res = compare('expval-biasedmf',travalpair_binary,['features'])
		print(res)
		op2 = res['ndcg'].loc[names[1]].idxmax()
			#ImplicitMF
		eval = MultiEval('impval', recommend=r)
		eval.add_datasets(travalpair_binary, name='ML-Small-binary')
		eval.add_algorithms([ImplicitMF(features=f) for f in [20,100,200]], attrs=['features'], name=names[2])
		eval.run()
		res = compare('impval',travalpair_binary,['features'])
		...
		#Test
		#Bias
		eval = MultiEval('exptest-bias', recommend=r)
		eval.add_datasets(tratestpair, name='ML-Small')
		eval.add_algorithms([Bias(damping=int(op1))],attrs=['damping'], name=names[0])
		eval.run()
		res = compare('exptest-bias',tratestpair_binary,['damping'])
		print("--- Test Performance ---")
		print(res)
		#BiasedMF
		eval = MultiEval('exptest-biasedmf', recommend=r)
		eval.add_datasets(tratestpair, name='ML-Small')
		eval.add_algorithms([BiasedMF(features=int(op2))], attrs=['features'], name=names[1])  
		eval.run()
		res = compare('exptest-biasedmf',tratestpair_binary,['features'])
		print(res)
		#ImplicitMF
		eval = MultiEval('imptest', recommend=r)
		eval.add_datasets(tratestpair_binary, name='ML-Small-binary')
		eval.add_algorithms([ImplicitMF(features=int(op3))], attrs=['features'], name=names[2])
		eval.run()
		res = compare('imptest',tratestpair_binary,['features'])
		print(res)
