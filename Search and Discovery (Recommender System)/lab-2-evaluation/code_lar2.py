...
if __name__ == '__main__':
	...
	for r in [10]:
		...
		op1 = res['ndcg'].loc[names[0]].idxmax()
			...
			#BiasedMF
		eval = MultiEval('expval-biasedmf-lar', recommend=r,eval_n_jobs=28 )
		eval.add_datasets(travalpair, name='ML')
		eval.add_algorithms([BiasedMF(features=f) for f in [20,100,200]], attrs=['features'], name=names[1])
		eval.run()
		res = compare('expval-biasedmf-lar',travalpair_binary,['features'])
		print(res)
		op2 = res['ndcg'].loc[names[1]].idxmax()
	
			#ImplicitMF
		eval = MultiEval('impval-lar', recommend=r,eval_n_jobs=28)
		...
		eval.run()
		res = compare('impval-lar',travalpair_binary,['features'])
		...
		#Bias
		eval = MultiEval('exptest-bias-lar', recommend=r)
		eval.add_datasets(tratestpair, name='ML')
		eval.add_algorithms([Bias(damping=int(op1))],attrs=['damping'], name=names[0])
		eval.run()
		res = compare('exptest-bias-lar',tratestpair_binary,['damping'])
		print("--- Test Performance ---")
		print(res)
		
		#BiasedMF
		eval = MultiEval('exptest-biasedmf-lar', recommend=r)
		eval.add_datasets(tratestpair, name='ML')
		eval.add_algorithms([BiasedMF(features=int(op2))], attrs=['features'], name=names[1])  
		eval.run()
		res = compare('exptest-biasedmf-lar',tratestpair_binary,['features'])
		print(res)

		#ImplicitMF
		eval = MultiEval('imptest-lar', recommend=r)
		...
		eval.run()
		res = compare('imptest-lar',tratestpair_binary,['features'])
		print(res)
