...
def create (spark, file_path):
    ...
    temp_train, temp_val, temp_test = user_list[:int(0.006*user_num)], user_list[int(0.006*user_num):int(0.008*user_num)], user_list[int(0.008*user_num):int(0.01*user_num)]
    ...
    val.write.parquet("goodread_validation_1.parquet")
    test.write.parquet("goodread_test_1.parquet")

    union1 = train_60.unionAll(val_add)
    union2 = union1.unionAll(test_add)
    train = union2.withColumn("index", monotonically_increasing_id())
    train.write.parquet('goodread_train_1.parquet')
